...
RUN_ROOT="${RUN_ROOT:-output/toy_runs/ddcl_objective_sweep_$(date +%Y%m%d_%H%M%S)}"
...
launch_job() {
  ...
  echo "Launching DDCL objective sweep: agent=${agent} variant=${variant} seed=${seed} log=${log_file}"
  mkdir -p "${hydra_dir}"
  (
...

{
  ...
  echo "cwd=$(pwd)"
  echo "env=${ENV_NAME}"
  echo "agents=ddcl_mse + ddcl_jepa_cosine"
  echo "seeds=${SEEDS}"
  echo "max_parallel=${MAX_PARALLEL}"
  echo "run_root=${RUN_ROOT}"
...
for seed in ${SEEDS}; do
  wait_for_slot
  launch_job ddcl_mse "mse-det-eval-wavg-targets-s35-l1e3" "${seed}" \
  ...
  wait_for_slot
  launch_job ddcl_mse "mse-det-eval-wavg-targets-s20-l3e4" "${seed}" \
    agent.ddcl_deterministic_eval=true \
    agent.ddcl_deterministic_targets=true \
    agent.plan_unc_prop_mode=weighted-avg \
    agent.unc_prop_mode=sample \
  ...
  wait_for_slot
  launch_job ddcl_mse "mse-det-eval-wavg-targets-s20-l1e4" "${seed}" \
    agent.ddcl_deterministic_eval=true \
    agent.ddcl_deterministic_targets=true \
    agent.plan_unc_prop_mode=weighted-avg \
    agent.unc_prop_mode=sample \
  ...
  wait_for_slot
  launch_job ddcl_mse "jepa-cosine-det-eval-wavg-targets-s35-l1e3" "${seed}" \
    agent.consistency_loss=cosine \
    agent.ddcl_deterministic_eval=true \
    agent.ddcl_deterministic_targets=true \
    agent.plan_unc_prop_mode=weighted-avg \
    agent.unc_prop_mode=sample \
  ...
  wait_for_slot
  launch_job ddcl_mse "jepa-cosine-det-eval-wavg-targets-s20-l3e4" "${seed}" \
    agent.consistency_loss=cosine \
    agent.ddcl_deterministic_eval=true \
    agent.ddcl_deterministic_targets=true \
    agent.plan_unc_prop_mode=weighted-avg \
  ...
  wait_for_slot
  launch_job ddcl_mse "jepa-cosine-det-eval-wavg-targets-s20-l1e4" "${seed}" \
    agent.consistency_loss=cosine \
    agent.ddcl_deterministic_eval=true \
    agent.ddcl_deterministic_targets=true \
    agent.plan_unc_prop_mode=weighted-avg \
...
echo "Completed all DDCL objective jobs. Logs: ${RUN_ROOT}/logs"
